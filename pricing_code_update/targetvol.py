...
class TVSForwardCurve(Curve):
    ...
    def __init__(self, reference = 0, vola_target = None, spot_price = None, strategy = None, mu = None,nu = None, discounting_curve = None, fees = None, fees_dates = None):
        ...
        self.T = fees_dates
        self.phi_t = interp1d(fees_dates, fees, kind='previous',fill_value="extrapolate", assume_sorted=False)
    ...
    def curve(self,date):
        ...
        if date.shape!=():
            print(self.alpha(0))
            return np.asarray([(self.I_0/self.D(extreme)) * exp(-quad_piecewise(l,self.alpha.T,self.reference,extreme,vectorial=1))*exp(-quad_piecewise(self.phi_t,self.T,self.reference,extreme)) for extreme in date])
        else:
            return (self.I_0/self.D(date)) * exp(-quad_piecewise(l,self.alpha.T,self.reference,date,vectorial=1))*exp(-quad_piecewise(self.phi_t,self.T,self.reference,date))
