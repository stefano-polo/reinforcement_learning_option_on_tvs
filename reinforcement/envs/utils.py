...
def sign_renormalization(vec: np.ndarray, sum_plus: float, sum_minus: float) -> np.ndarray:
    """
...
def build_allocation_time_grid(maturity: float, allocation_frequency: str = 'monthly', day_count_convention: str = "ACT_365"):
    """
    ...
    if allocation_frequency == 'monthly':
        month_days_lenght = np.array([31., 28., 31., 30., 31., 30., 31., 31., 30., 31., 30., 31.])
        ...
        observation_time_grid = np.linspace(1.0 / number_days_in_year, maturity, int(maturity * number_days_in_year))
