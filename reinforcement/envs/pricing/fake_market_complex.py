...
def load_fake_market(N_equity, r, maturity):
    ...
    if N_equity ==3:
        ...
        repo_rate3 = (-1)*np.array([0.03,0.022,0.04])/100
    else:
    ...
    repo_rate1 = (-1)*np.array([0.03,0.03,0.03])/100
    T_repo1 = np.array([1/12,4./12,maturity])       #data observation of the market repo rates for equity 1
    repo_rate2 = (-1)*np.array([0.04,0.03,0.035])/100  #market repo rates for equity 1 0.52
    T_repo2 = np.array([1/12.,4/12.,maturity])

    sigma1 = np.array([20,24.,23.])/100
