...
class TVS_LV_ENV_reward2(gym.Env):
    ...
    def __init__(self, market_folder: str = "../market_data/FakeSmilesDisplacedDiffusion", asset_names: list or tuple = ["DJ 50 EURO E", "S&P 500 NET EUR"],
        ...
        self.intermediate_discounts = discounting_curves(self.T) / discounting_curves(self.observation_grid)
        self.mu_function = Drift(forward_curves)
        self.mu_time_grid = np.append(self.mu_function.T[self.mu_function.T < self.T], self.T)
        ...
        self.r_t_in_euler_grid = discounting_curves.r_t(np.append(0., euler_time_grid)) # shape (N_euler_grid,) insert the referencedate and discard the last euler grid point
        ...
        if neural_network_action_parameterization is None:  # if no neural network action parameterization then the allocation strategy coincides with the NN output
            ...
            self.mu_values_on_euler_grid = self.mu_function(np.append(0., euler_time_grid[:-1]))
            ...
            if action_constraints == allowed_action_constraints[0]:
                alpha.optimization_constrained(mu=self.mu_function, nu=nu_function, n_trial=500, typo=1)
    ...
    def step(self, action: np.ndarray) -> tuple:
                ...
                if self.parameterized_action:
                    if self.long_only:
                        if self.start_from_baseline:
                            parameterization = optimization_only_long(self.mu_values_on_euler_grid[euler_grid_index], nu_matrix, n_trial=1, guess=action)
        ...
        if self.free_allocation_bounds:
            black_optimal_allocation = Markowitz_solution(self.mu_function(self.current_time), nu_matrix, -1)
        elif self.long_only:
            black_optimal_allocation = optimization_only_long(self.mu_values_on_euler_grid[euler_grid_index], nu_matrix, n_trial=2)
        ...
        local_drift = np.sum(np.sum(black_optimal_allocation * mu_values, axis=1) * np.diff(grid)) * omega_coefficient
        forward_tvs = self.I_t * (np.exp(-local_drift) / self.intermediate_discounts[self.time_index])
        if self.T - self.current_time > 1e-14:
            self.V_t_plus_1 = BS_European_option_closed_form(forward_tvs, self.strike_opt, self.T - self.current_time,
                                                             self.intermediate_discounts[self.time_index], self.target_vol, 1)
        else:
            self.V_t_plus_1 = np.maximum(forward_tvs - self.strike_opt, 0.)
