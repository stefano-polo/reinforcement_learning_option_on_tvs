...
class TVS_enviroment(gym.Env):
    ...
    def __init__(self, filename= "TVS_example.xml", spot_I = 100, target_volatility=0.1,strike_opt=100., maturity=1., constraint = "free", action_bound = 25/100, sum_long=None, sum_short=None):
        ...
        self.correlation_chole = np.linalg.cholesky(self.correlation_matrix)
        ...
        self.model = Black(fixings = self.time_grid, forward_curve = self.F, variance = self.V)
        ...
        for t in self.time_grid[1:]:
            self.vola_t = np.vstack([self.vola_t,sqrt(np.sum(self.nu(t)**2,axis=0))])
