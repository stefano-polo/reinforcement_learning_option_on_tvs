from plot_utilities import plot_rolling, pu, join_curves
...
discount_factor = 1.0097815717231273  # discount factor used to discount the learning curves
...
MOVING_AVERAGE_WINDOW = int(1000)  # window of the moving average used to smooth the learning curves (measured in episodes)
...
WHICH_LOGS = [  # ("name of the output folder", "legend label"). If join_learning is True then the legend label is ignored.
    ('ppo2_1e6_5x8_3e-4_trainingseed45891_beta0.7_copy_tanhpricing_prova', 'learning_curve_1'),
    ('ppo2_1e5_5x8_3e-4_trainingseed45891_beta0.7_copy_tanhpricing_prova', 'learning_curve_2'),
]
...
if join_learning:
    ...
    plot_rolling(steps_joined, rewards_joined * discount_factor, MOVING_AVERAGE_WINDOW, WHICH_LOGS[0][1], n_sigma)
else:
    for log, title in WHICH_LOGS:
        ...
        plot_rolling(steps, rewards * discount_factor, MOVING_AVERAGE_WINDOW, title, n_sigma)


plt.axhline((0.04149584726740925 + n_sigma * 5.089556147059726e-05), color='red', lw=2.5, linestyle='-.', label="Baseline Strategy")
plt.axhline((0.04149584726740925 - n_sigma * 5.089556147059726e-05), color='red', lw=2.5, linestyle='-.')
