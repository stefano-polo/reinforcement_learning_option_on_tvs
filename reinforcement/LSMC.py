...
def alpha_vector(a1, nu, Nsim=1, norm=1):
    ...
    #numerator = norm*(nu[1,0]**2+nu[1,1]**2)
    #denominator = (nu[0,0,:]*nu[1,1,:])**2+(nu[1,0,:]*nu[0,1,:])**2-2.*nu[0,0,:]*nu[1,1,:]*nu[1,0,:]*nu[0,1,:]
...
#alpha_grid = np.linspace(-1.,1.,number_alpha)
alpha_grid = np.linspace(0.01, np.pi*2, number_alpha) 
...
coeff_matrices = np.zeros((len(I_grid),len(t_grid[:-1]), len(alpha_grid), Polynomial_grade+1))
...
for t in reversed(t_grid[:-1]):
    ...
    for It in I_grid:
        ...
        for a in alpha_grid:
            A = a*np.ones(Nsim)
            alpha = np.array(([R*np.cos(A),R*np.sin(A)])).T#alpha_vector(a, nu, Nsim, norm=5)# np.array(([A, np.sqrt(1.-A**2)])).T
           # print(alpha)
            #print(alpha)
            ...
            coeff_matrices[index_I_grid, len(t_grid[:-1])-k-1, alpha_index,:] = coeff_It_t   #saving regression coeff
            ##Continuation Value
            V_alpha = np.sum(coeff_It_t * Y, axis=1)      #continuation value
           # print("Continuation ",V_alpha)
...
for i in range(len(t_grid[:-1])):
    ...
    coeff_matrices_time =  coeff_matrices[:,i,:,:]      #shape(np.zeros((len(I_grid),len(t_grid), len(alpha_grid), Polynomial_grade+1)))
    ...
    for j in range(N_euler_grid):
        ...
        if j==0:
            alpha_best = np.array(([R*np.cos(alpha_chosen),R*np.sin(alpha_chosen)])).T#alpha_vector(a, nu, Nsim, norm=5)# np.array(([A, np.sqrt(1.-A**2)])).T
