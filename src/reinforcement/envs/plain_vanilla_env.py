...
class PlainVanillaOption(gym.Env):
    ...
    def __init__(
        ...
        zero_interest_rate: float = 1 / 100,
        volatility: float = 20 / 100,
        option_strike: float = 100.0,
        option_maturity: float = 1.0,
        call_put: int = 1,
        n_sim_for_cache: int = int(1e4),
    ) -> None:
        """
        ...
        t_max = 10.0
        zero_interest_rate = array(
            [zero_interest_rate, zero_interest_rate, zero_interest_rate]
        )
        ...
        self.discounting_curve = DiscountingCurve(
            reference=0, discounts=d, discount_dates=zero_interest_rate_dates
        )
        self.forward_curve = EquityForwardCurve(
            reference=0,
            spot=spot_price,
            discounting_curve=self.discounting_curve,
            repo_dates=array([0.0, t_max]),
            repo_rates=array([0.0, 0.0]),
        )
        ...
        self.observation_space = spaces.Box(
            low=array([0.0, 0.0]),
            high=array([spot_price * 10000.0, self.T + 1.0 / 365.0]),
        )
        self.seed()
        self.reset()
    ...
    def theoretical_price(self) -> float:
        return BS_European_option_closed_form(
            ...
            discount=self.discounting_curve(self.T),
            volatility=np.sqrt(self.variance_curve(self.T)),
            call_put=self.cp,
        )
