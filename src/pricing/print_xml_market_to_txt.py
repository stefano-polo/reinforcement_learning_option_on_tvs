...
reference = market.get(
    f"market.{discount_curve}.zero_curve.{discount_curve} :STD.reference_date"
)
dates = np.array(
    market.get(f"market.{discount_curve}.zero_curve.{discount_curve} :STD.data.dates")
)
discounts = np.array(
    market.get(
        f"market.{discount_curve}.zero_curve.{discount_curve} :STD.data.discounts"
    )
)
day_count_convention = market.get(
    f"market.{discount_curve}.zero_curve.{discount_curve} :STD.convention.day_count"
)

if day_count_convention == "ACT365":
    discount_dates = (dates - reference) / 365.0
elif day_count_convention == "ACT360":
    discount_dates = (dates - reference) / 360.0
else:
...
correlation_matrix = np.array(
    market.get("market.data.correlation_matrix.data.correlations")
)
asset_names_in_correlation = list(
    market.get("market.data.correlation_matrix.data.names")
)
correlation_dataframe = pd.DataFrame(
    correlation_matrix,
    columns=asset_names_in_correlation,
...
for name in asset_names:
    market_volatilities = np.array(
        market.get("market." + name + ".volatility.data.volatilities")
    )
    maturities = np.array(
        market.get("market." + name + ".volatility.data.expiry_dates")
    )
    vola_reference = market.get("market." + name + ".volatility.reference_date")
    maturities = (maturities - vola_reference) / 365.0
    strikes = np.array(market.get("market." + name + ".volatility.data.strikes"))
    spot = market.get("market." + name + ".forwards.data.spot")
    try:
        repo_dates = np.array(
            market.get("market." + name + ".forwards.data.repo.dates")
        )
        repo_day_count_convention = market.get(
            "market." + name + ".forwards.convention.repo.day_count"
        )
        repo_reference = market.get("market." + name + ".forwards.reference_date")
        if repo_day_count_convention == "ACT365":
            repo_dates = (repo_dates - repo_reference) / 365.0
        elif repo_day_count_convention == "ACT360":
            repo_dates = (repo_dates - repo_reference) / 360.0
        else:
            raise ValueError(
                f" {day_count_convention} day count convention not supported"
            )
        repo_rates = np.array(
            market.get("market." + name + ".forwards.data.repo.rates")
        )
    except:
        warning_string = "No repo data for " + name + ". Setting repo_rates to zero"
        warnings.warn(warning_string)
        repo_dates = np.array([0.0, 10.0, 20.0])
        repo_rates = np.array([0.0, 0.0, 0.0])
    np.savetxt(saving_folder + "spot_data" + name + ".txt", np.append(spot, spot))
    np.savetxt(
        saving_folder + "repo_data_" + str(name) + ".txt", (repo_dates, repo_rates)
    )
    ...
    try:
        lv_dates = np.array(
            market.get("model.local_volatility.parameters." + name + ".expiry_yrf")
        )
        lv_money = np.array(
            market.get(
                "model.local_volatility.parameters." + name + ".local_volatility_money"
            )
        )
        lv_pars = np.array(
            market.get(
                "model.local_volatility.parameters." + name + ".local_volatility_pars"
            )
        )
