...
class DiscountingCurve(Curve):
    ...
    def __init__(self, reference: int or float, discounts: np.ndarray, discount_dates: np.ndarray, day_count_convention: str = None) -> None:
        """
        ...
        assert discounts.ndim == discount_dates.ndim == 1  # discount and discount_dates must be 1D arrays
        assert len(discounts) == len(discount_dates)  # check that the number of discounts and dates are the same
        self.T = DayCountConversion(self.reference, discount_dates, day_count_convention)  # convert dates into fixings (yrf)
        if self.T[0] == 0:
            self.T = self.T[1:]
            discounts = discounts[1:]
        r_zero = (-1.0 / self.T) * log(discounts)
        r_instant = np.append(r_zero[0], (1. / (self.T[:-1] - self.T[1:])) * log(discounts[1:] / discounts[:-1]))
        self.R = interp1d(self.T, r_zero, fill_value="extrapolate")  # zero interest rate from 0 to T1 (linear interpolation)
        self.r_t = interp1d(self.T, r_instant, kind='next', fill_value="extrapolate")  # instant interest rate (piecewise constant)
...
class EquityForwardCurve(Curve):
    ...
    def __init__(self, reference: int or float, spot: float, discounting_curve: DiscountingCurve, repo_rates: np.ndarray = None,
                 repo_dates: np.ndarray = None, day_count_convention: str = None, asset_name: str = None) -> None:
        """
        ...
        if repo_rates is None and repo_dates:
            repo_rates = np.array([0., 0.])
            repo_dates = np.array([0. + reference, 10. + reference])
            day_count_convention = None  # if repo_rates is None then the repo_dates are assumed to be in yrf
        elif (repo_rates is not None and repo_dates is None) or (repo_rates is None and repo_dates is not None):
            raise ValueError("repo_rates and repo_dates must be both None or both not None")
        assert len(repo_rates) == len(repo_dates)
        assert repo_rates.ndim == repo_dates.ndim == 1  # repo_rates and repo_dates must be 1D arrays
        assert spot > 0.  # spot price must be positive
        self.asset_name = asset_name
        self.spot = spot
        self.reference = reference
        self.discounting_curve = discounting_curve
        self.T = DayCountConversion(self.reference, repo_dates, day_count_convention)  # convert dates into fixings (yrf)
        if self.T[0] != 0.:  # the
            self.T = np.append(0., self.T[:-1])
        self.q_values = np.array([repo_rates[0]])  # array of the instant hedging costs
        for i in range(1, len(self.T)):
            instant_repo_rate = (self.T[i] * repo_rates[i] - self.T[i - 1] * repo_rates[i - 1]) / (self.T[i] - self.T[i - 1])
            self.q_values = np.append(self.q_values, instant_repo_rate)
        self.q = interp1d(self.T, self.q_values, kind='previous', fill_value="extrapolate", assume_sorted=False)  # instant hedging costs (piecewise constant right open)
...
class ForwardVariance(Curve):
    ...
    def __init__(self, reference: int or float, market_volatility_matrix: np.ndarray, strikes: np.ndarray, maturity_dates: np.ndarray,
                 strike_interp: float or EquityForwardCurve, day_count_convention: str = None, asset_name: str = None) -> None:
        """
        Forward Variance Curve.
        ...
        assert strikes.ndim == maturity_dates.ndim == 1  # strikes and maturity_dates must be 1D arrays
        assert len(market_volatility_matrix.T) == len(strikes)
        assert len(market_volatility_matrix) == len(maturity_dates)
        assert type(strike_interp) in [float, EquityForwardCurve, np.float64, np.float32]
        self.asset_name = asset_name
        self.reference = reference
        self.market_volatility = market_volatility_matrix
        self.T = DayCountConversion(self.reference, maturity_dates, day_count_convention)  # convert dates into fixings (yrf)
        if isinstance(strike_interp, EquityForwardCurve):
            ...
            self.K = strikes
            matrix_interpolated = interp1d(strikes, market_volatility_matrix, axis=1)(strike_interp(self.T))
            ...
            self.spot_vol = interp1d(strikes, market_volatility_matrix, axis=1)(strike_interp)

        self.forward_vol = np.array([self.spot_vol[0]])  # forward (instantaneous) volatility from 0 to T1
        for i in range(1, len(self.T)):
            forward_volatility = (self.T[i] * (self.spot_vol[i]**2) - self.T[i-1] *
...
class LocalVolatilityCurve:
    ...
    def __init__(self, local_volatility_parameters: np.ndarray, moneyness_matrix: np.ndarray, maturities: np.array, asset_name: str = None, log_money_interpolation_rule: str = "kruger"):
        """
        Local Volatility Curve.
        ...
        """
        assert local_volatility_parameters.ndim == moneyness_matrix.ndim == 2  # local_volatility_parameters and moneyness_matrix must be 2D arrays
    ...
    def interpolate_on_time(self, simulation_time_grid: np.ndarray) -> None:
        ...
        self.time_indexes_of_simulation_grid = self.time_interpolator(simulation_time_grid).astype(int)
        self.interpolator_strikes = []
        self.money_max = np.array([])
        self.money_min = np.array([])
        self.LV_max = np.array([])
        ...
        for i in range(self.time_indexes_of_simulation_grid[-1] + 1):
            ...
            if self.use_kruger:
                ...
                self.interpolator_strikes.append(interp1d(this_money, this_lv, kind="nearest", fill_value="extrapolate"))
    ...
    def get_time_index_grid(self) -> np.ndarray:
        ...
        if self.time_indexes_of_simulation_grid is None:
            raise ValueError("The simulation time grid has not been interpolated yet. Use interpolate_on_time method.")
        return self.time_indexes_of_simulation_grid
            
    def vectorized_call(self, index: int, log_k: float or np.ndarray) -> float or np.ndarray:
        """
    ...
    def __call__(self, t: float, log_k: float) -> float:
        ...
        if self.use_kruger:
            ...
            eta = interp1d(this_money, this_lv, kind="nearest", fill_value="extrapolate")(log_k)
        eta[log_k < this_money[0]] = this_lv[0]  # extrapolation rule (constant in log-moneyness)
        eta[log_k > this_money[-1]] = this_lv[-1]  # extrapolation rule (constant in log-moneyness)
        return eta
...
class Black:
    ...
    def __init__(self, fixings: np.ndarray, forward_curve: EquityForwardCurve or list, variance_curve: ForwardVariance or list,
                 correlation_matrix: np.ndarray = None, sampling: str = "standard") -> None:
        """
        Black and Scholes model.
        ...
        if type(forward_curve) == list and len(forward_curve) > 1:
            ...
            if correlation_matrix is None:  # if no correlation matrix is provided, we assume no correlated assets
            ...
            for i in range(self.n_stocks):
                ...
                for j in range(self.n_fixings):
                    if j == 0:
                        self.variance_values[i, j] = quad_piecewise(variance_curve[i], variance_curve[i].T, 0., fixings[j])  # variance from t=0 to t=fixings[j]
                    else:
                        self.variance_values[i, j] = quad_piecewise(variance_curve[i], variance_curve[i].T, fixings[j-1], fixings[j]) # variance from t=fixings[j-1] to t=fixings[j]
        else:
            ...
            for j in range(self.n_fixings):
                if j == 0:
                    self.variance_values[j] = quad_piecewise(variance_curve, variance_curve.T, 0., fixings[j])
                else:
                    self.variance_values[j] = quad_piecewise(variance_curve, variance_curve.T, fixings[j-1], fixings[j])
        if sampling == "standard":
            self.antithetic_sampling = False
        elif sampling == "antithetic":
            self.antithetic_sampling = True
        else:
            raise ValueError("Sampling type not recognized. Please use 'standard' or 'antithetic'.")


    def simulate(self, random_generator: np.random = None, n_sim: int or float = 1, seed: int = 14, return_log_martingale: bool = False) -> np.ndarray:
        """
        ...
        if self.n_stocks == 1:
            ...
            for i in range(self.n_fixings):
                ...
                if i == 0:
                    logmartingale[:, i] = -0.5 * self.variance_values[i] + sqrt(self.variance_values[i]) * Z
                else:
                    logmartingale[:, i] = logmartingale[:, i - 1] - 0.5 * self.variance_values[i] + sqrt(self.variance_values[i]) * Z
            ...
            for i in range(self.n_fixings):
                ...
                for j in range(self.n_stocks):
                    if i == 0:
                        logmartingale[:, i, j] = - 0.5 * self.variance_values[j, i] + sqrt(self.variance_values[j, i]) * ep[j]
                    elif i != 0:
                        logmartingale[:, i, j] = logmartingale[:, i-1, j] - 0.5 * self.variance_values[j, i] + sqrt(self.variance_values[j, i]) * ep[j]
            if return_log_martingale:
                return logmartingale
            else:
                return exp(logmartingale) * self.forward_values
...
class LV_model:
    ...
    def __init__(self, fixings: np.array, forward_curve: EquityForwardCurve or list, local_vol_curve: LocalVolatilityCurve or list, n_euler_grid: int = 100,
                 correlation_matrix: np.ndarray = None, sampling: str = "standard", return_grid_values_for_tvs: bool = False) -> None:
        """
        Local Volatility Model.
        The class implements both the single asset and multi-asset Black and Scholes models depending on the input.
        ...
        self.euler_time_grid, self.dt = get_euler_grid(fixings, n_euler_grid)  # get the Euler grid for the model simulation and the time step of each
        ...
        self.sqrt_dt = np.sqrt(self.dt)  # squared time interval (needed for Euler Scheme) (it can be computed just one time in the constructor)
        ...
        if type(forward_curve) == list and len(forward_curve) > 1:
            ...
            for i in range(self.n_stocks):
                if self.return_grid_values_for_tvs:
                    ...
                    self.forward_values[:, i] = forward_curve[i](fixings)
                self.local_vol[i].interpolate_on_time(self.euler_time_grid)  # interpolate the local volatility curve on the Euler grid
                time_indexes = self.local_vol[i].get_time_index_grid()  # get the time indexes of the local volatility curve used to interpolate on the Euler grid
                if i == 0:
            ...
            self.n_stocks = 1
            self.forward_values = forward_curve(fixings)
            self.local_vol.interpolate_on_time(self.euler_time_grid)  # interpolate the local volatility curve on the Euler grid
            self.time_indexes = self.local_vol.get_time_index_grid()  # get the time indexes of the local volatility curve used to interpolate on the Euler grid
            if return_grid_values_for_tvs:
                raise ValueError("return_grid_values_for_tvs is not available for the single-asset local volatility model.")
    ...
    def simulate(self, random_generator: np.random = None, n_sim: int = 1, seed: int = 14) -> np.ndarray or tuple:
        """
       ...
       """
        ...
        if self.n_stocks == 1:
            ...
            for i in range(self.n_time_grid):
                ...
                if i == 0:
                    ...
                    vol = self.local_vol.vectorized_call(self.time_indexes[i-1], log_X_t)
                    log_X_t = log_X_t - 0.5 * dt * (vol**2) + vol * dt_root_squared * Z
                counter = i+1
        ...
        # Multi Asset simulation
            ...
            if not self.return_grid_values_for_tvs:
                log_martingale_at_fixing_dates = np.zeros((n_mc_sim, self.n_fixings, self.n_stocks))
            else:
                instant_log_martingale = np.zeros((n_mc_sim, self.n_time_grid, self.n_stocks))
                instant_vola = np.zeros((n_mc_sim, self.n_time_grid, self.n_stocks))

            log_X_t = np.zeros((n_mc_sim, self.n_stocks)); vol = np.zeros((n_mc_sim, self.n_stocks));
            for i in range(self.n_time_grid):
                ...
                for j in range(self.n_stocks):
                    ...
                    if i == 0:
                        ...
                        log_X_t[:, j] = -0.5 * dt * (vol[:, j]**2) + vol[:, j] * dt_root_squared * ep[j]
                    elif i != 0:
                        vol[:, j] = self.local_vol[j].vectorized_call(self.time_indexes_matrix[j, i-1], log_X_t[:, j])
                        log_X_t[:, j] = log_X_t[:, j] - 0.5 * dt * (vol[:, j]**2) + vol[:, j] * dt_root_squared * ep[j]

                if self.return_grid_values_for_tvs:  # save the values of the martingale at the Euler grid fixing dates
...
def Vanilla_PayOff(simulated_paths: np.ndarray, strike: float, call_put: float = 1.0, sampling: str = "standard") -> np.ndarray:
    """
    ...
    if sampling == "standard":
        ...
        raise ValueError("Sampling method not recognized (available methods: standard, antithetic)")
