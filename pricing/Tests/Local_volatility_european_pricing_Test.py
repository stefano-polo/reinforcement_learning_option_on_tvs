...
from pricing import LocalVolatilityCurve, EquityForwardCurve, ForwardVariance, DiscountingCurve,Vanilla_PayOff,LV_model
...
asset_names_list = "DJ 50 TR", "S&P 500 NET EUR","MSCI EM MKT EUR","I NKY NTR EUR","FTSE100 NTR E","SMI TR EUR","DAX 30 E","FTSEMIBN","HSI NTR EUR"
D, F, V, LV, correlation_matrix = LoadFromTxt(asset_names_list,folder, "ATM_FWD", local_vol_model=True)
F = F[idx]
V = V[idx]
LV = LV[idx]
...
IV_curve = LocalVolatilityCurve(spot_volatilities,money_ness,maturities,V.asset_name, "piecewise")
# -
...
N_simulation = 1e5
expiries = LV.T
model = LV_model(fixings=expiries, local_vol_curve=LV, forward_curve=F, n_euler_grid = N_grid, sampling=sampling, return_grid_values_for_tvs=False)
...
for j in range(n_expiries):
    for i in range(n_strikes):
        ...
        imp_volatility[i,j] = Price_to_BS_ImpliedVolatility(expiries[j], E_X[j], k, result[j], kind, 1.0)
        imp_volatility_plus[i,j] = Price_to_BS_ImpliedVolatility(expiries[j], E_X[j], k, result[j] + err_result[j], kind, 1.0)
        imp_volatility_minus[i,j] = Price_to_BS_ImpliedVolatility(expiries[j], E_X[j], k, result[j] - err_result[j], kind, 1.0)
...
for i in range(n_expiries):
    ...
    plt.plot(this_moneyness,imp_volatility[:,i] + n_sigma * err_upper, 'r--',label='Monte Carlo')
    plt.plot(this_moneyness, imp_volatility[:,i] - n_sigma * err_lower, 'r--')
    plt.plot(this_moneyness,IV_curve(expiries[i],np.log(this_moneyness)),label='Market')
