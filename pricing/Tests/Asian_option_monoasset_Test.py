...
from closedforms import Price_to_BS_ImpliedVolatility, GA_Asian_option_closed_form, BS_European_option_closed_form, volatility_asian, interest_rate_asian
...
V = ForwardVariance(reference=t,maturity_dates=spot_vol_dates,strikes=K_spot_vola,market_volatility_matrix=spot_vol,strike_interp=spot_price)
...
E_X = np.mean(X_t)
K_norm = 1
pay_normalized = Vanilla_PayOff(X_t, K_norm, call_put = kind, sampling=sampling) #ATM forward pricing

"""Calculating closed form"""
right = GA_Asian_option_closed_form(F(maturity), forward_g_asian, maturity, D(maturity), volatility, N_averages, kind)
# -
...
for i in range (N_block):
    imp_volatility_mean[i] = Price_to_BS_ImpliedVolatility(maturity, E_X, 1.0, result[i], kind, 1.0)
    imp_volatility_plus[i] = Price_to_BS_ImpliedVolatility(maturity, E_X, 1.0, result[i] + result_err[i], kind, 1.0)
    imp_volatility_minus[i] = Price_to_BS_ImpliedVolatility(maturity, E_X, 1.0, result[i] - result_err[i], kind, 1.0)
...
for i in range(len(maturities)):
    ...
    for j in range(len(logmoneyness)):
        ...
        imp_volatility[j][i] =  Price_to_BS_ImpliedVolatility(maturities[i], E, K, result, kind, 1.0)
        imp_volatility_plus[j][i] = Price_to_BS_ImpliedVolatility(maturities[i], E, K, result + err_result, kind, 1.0)
        imp_volatility_minus[j][i] = Price_to_BS_ImpliedVolatility(maturities[i], E, K, result - err_result, kind, 1.0)
...
# +
plt.figure(figsize=(22,13))
plt.suptitle("Geometric Average Asian Option: Implied volatility at fixed maturity", fontsize=18)
n_sigma = 2   #how many sigma for the errorbars
