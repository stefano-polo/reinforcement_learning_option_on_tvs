...
from pricing import EquityForwardCurve, ForwardVariance, DiscountingCurve,Vanilla_PayOff
...
asset_names_list = "DJ 50 TR", "S&P 500 NET EUR","MSCI EM MKT EUR","I NKY NTR EUR","DAX 30 E", "FTSE100 NTR E","SMI TR EUR","FTSEMIBN", "CAC 40 NTR", "HSI NTR EUR"
D, F, V, correlation_matrix = LoadFromTxt(asset_names_list,folder, strike_interpolation_rule="ATM_FWD", local_vol_model=False)
...
TVSF = TVSForwardCurve(reference = 0., vola_target =vol_target, spot_price = I_0, strategy = alpha, mu = mu,nu = nu, discounting_curve = D)
...
for i in range (N_block):
    imp_volatility_mean[i] = Price_to_BS_ImpliedVolatility(dates[n], 1.0, K/forward[n], result[i], kind, 1.0)
    imp_volatility_plus[i] = Price_to_BS_ImpliedVolatility(dates[n], 1.0, K/forward[n], result[i] + result_err[i], kind, 1.0)
    imp_volatility_minus[i] = Price_to_BS_ImpliedVolatility(dates[n], 1.0, K/forward[n], result[i] - result_err[i], kind, 1.0)
...
y1 = np.array([result_intuitive1[N_block-1],result_intuitive2[N_block-1],result_intuitive3[N_block-1], result_opt1[N_block-1]])
...
# +
# %%time
alpha.optimization_constrained(mu=mu,nu=nu,n_trial=10, long_limit=10/100, constraint_strategy=2)
...
# +
# %%time
alpha.optimization_constrained(mu=mu,nu=nu,n_trial=10, long_limit=90/100, short_limit=10/100, constraint_strategy=3)
