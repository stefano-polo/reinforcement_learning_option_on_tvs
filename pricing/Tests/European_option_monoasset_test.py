...
F = EquityForwardCurve(reference_date, spot_price, D, repo_dates=np.array([0.,T_max]), repo_rates=np.array([0.1/100,0.1/100]))
...
call_black = BS_European_option_closed_form(F(maturities), F(maturities), maturities, D(maturities), volatility, kind)
# -
...
fig, ax = plt.subplots(1, 1, figsize=(7, 5))
plt.title(r"log$(K/F)$ = "+str(log(K_norm))+" -- Maturity = "+str(round(maturities[n],1))+' [yr]',fontsize=13)
