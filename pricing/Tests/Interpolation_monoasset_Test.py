...
D, F, V, correlation_matrix = LoadFromTxt(asset_names_list,folder, "ATM_FWD", local_vol_model=False)
F = F[idx]
V = V[idx]
...
for i in range (len(T)):
    imp_volatility[i] = Price_to_BS_ImpliedVolatility(T[i], E[i], strike, result[i], call_put, 1.0)
    imp_volatility_minus[i] = Price_to_BS_ImpliedVolatility(T[i], E[i], strike, result[i] - err[i], call_put, 1.0)
    imp_volatility_plus[i] = Price_to_BS_ImpliedVolatility(T[i], E[i], strike, result[i] + err[i], call_put, 1.0)
...
fig, ax = plt.subplots(1, 1, figsize=(7, 5))
n_sigma = 1.0
plt.errorbar(T,abs(imp_volatility-V.spot_vol),yerr=[n_sigma*y_lower,n_sigma*y_upper], fmt = "o", label = "Monte Carlo")
