...
from montecarlo import MC_results, MC_Data_Blocking
from numpy import exp, log
import matplotlib.pyplot as plt
from closedforms import Price_to_BS_ImpliedVolatility, GAM_Basket_option_closed_form, volatility_basket, forward_basket
...
D = DiscountingCurve(reference=t, discounts=d, discount_dates=zero_interest_rate_dates)   #discounting curve
...
for i in range(N_equity):
    F.append(EquityForwardCurve(reference=t,spot=spot_price[i],discounting_curve=D,repo_rates=np.array([repos[i],repos[i]])/100,repo_dates=np.array([0.,T_max])))
...
for i in range(N_equity):
    ...
    K = globals()["K" + str(i+1)]
    V.append(ForwardVariance(reference=t,market_volatility_matrix=s_vola.T,strikes=K,maturity_dates=T_vola,strike_interp=spot_price[i]))
...
model = Black(maturities, forward_curve=F, variance_curve=V, correlation_matrix=corr, sampling=sampling)
...
K_norm = 1
pay_normalized = Vanilla_PayOff(X_t, K_norm, kind, sampling=sampling)  #ATM forward pricing

"""Calculating closed form"""
call_black = GAM_Basket_option_closed_form(F, f, maturities, D(maturities), volatility, corr, kind)
# -
...
for i in range (N_block):
    imp_volatility_mean[i] = Price_to_BS_ImpliedVolatility(maturities[n], E_X[n], 1.0, result[i], kind, 1.0)
    imp_volatility_plus[i] = Price_to_BS_ImpliedVolatility(maturities[n], E_X[n], 1.0, result[i] + result_err[i], kind, 1.0)
    imp_volatility_minus[i] = Price_to_BS_ImpliedVolatility(maturities[n], E_X[n], 1.0, result[i] - result_err[i], kind, 1.0)
...
font=12
fig, ax = plt.subplots(1, 1, figsize=(7, 5))
plt.title("log$(K/F)$ = "+str(log(K_norm))+" -- Maturity = "+str(round(maturities[n],1))+' [yr]',fontsize=font)
...
for i in range(len(logmoneyness)):
    ...
    for j in range(len(maturities)):
        imp_volatility[i][j] = Price_to_BS_ImpliedVolatility(maturities[j], E_X[j], K, result[j], kind, 1.0)
        imp_volatility_plus[i][j] = Price_to_BS_ImpliedVolatility(maturities[j], E_X[j], K, result[j] + err_result[j], kind, 1.0)
        imp_volatility_minus[i][j] = Price_to_BS_ImpliedVolatility(maturities[j], E_X[j], K, result[j] - err_result[j], kind, 1.0)
...
plt.suptitle("Geometric Average Basket Option: Implied volatility at fixed log-moneyness",fontsize=18)
...
# +
plt.figure(figsize=(22,13))
plt.suptitle("Geometric Average Basket Option: Implied volatility at fixed maturity",fontsize=18)
n_sigma = 2.55   #how many sigma for the errorbars
