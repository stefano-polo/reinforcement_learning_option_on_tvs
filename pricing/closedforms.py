...
def d1(forward: float or np.ndarray, strike: float or np.ndarray, time_to_maturity: float or np.ndarray, volatility: float or np.ndarray) -> float or np.ndarray:
    """
...
def BS_European_option_closed_form(forward: float or np.ndarray, strike: float or np.ndarray, time_to_maturity: float or np.ndarray, discount: float or np.ndarray, volatility: float or np.ndarray, call_put: float = 1.0) -> float or np.ndarray:
    """
    ...
    return discount * call_put * (forward * si.norm.cdf(d_1, 0.0, 1.0) - strike * si.norm.cdf(d_2, 0.0, 1.0))


def Price_to_BS_ImpliedVolatility(time_to_maturity: float, forward: float, strike: float, option_price: float, call_put: float, discount: float) -> float:
    """
    ...
    try:
        ...
        return BS_ImpliedVol(time_to_maturity, forward, strike, option_price, call_put, discount)
    except ImportError:
        try:
            import lets_be_rational.LetsBeRational as lbr
            return lbr.implied_volatility_from_a_transformed_rational_guess(option_price / discount, forward, strike, time_to_maturity, call_put)
        except:
            raise ImportError("pyGem and LetsBeRational are not installed. Please install pyGem and LetsBeRational to use this function.")
...
def interest_rate_asian(n_averages: int, zero_interest_rate: float, volatility: float) -> float:
    """
...
def GA_Asian_option_closed_form(forward: float, strike: float, time_to_maturity: float, discount: float, volatility: float, n_averages: int = 10, call_put: float = 1.0) -> float:
    """
    ...
    vol_asian = volatility_asian(n_averages, volatility)
    return BS_European_option_closed_form(forward_asian, strike, time_to_maturity, discount, vol_asian, call_put)


def volatility_basket(volatility_assets: np.ndarray, correlation_matrix: np.ndarray) -> float:
    """
    ...
    """
    assert len(volatility_assets) == len(correlation_matrix) == len(correlation_matrix.T)
...
def forward_basket(forwards_list: list, volatility_assets: np.ndarray, correlation_matrix: np.ndarray, time_to_maturity: float) -> float:
    """
    ...
    for i in range(n_assets):
        fwd_basket = fwd_basket * forwards_list[i](time_to_maturity) * exp((-0.5 * volatility_assets[i]**2) * time_to_maturity)
    fwd_basket = fwd_basket**(1.0 / n_assets)
    fwd_basket = fwd_basket * exp(0.5 * (sigma_basket**2) * time_to_maturity)
    return fwd_basket


def GAM_Basket_option_closed_form(forwards_list: list, strike: float, time_to_maturity: float, discount: float, volatility_array: np.array, correlation: np.ndarray, call_put: float = 1.0) -> float:
    """
    ...
    basket_fwd = forward_basket(forwards_list, volatility_array, correlation, time_to_maturity)
    return BS_European_option_closed_form(basket_fwd, strike, time_to_maturity, discount, vol_basket, call_put)
