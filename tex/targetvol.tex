...
In our work we keep fixed to the default values of \textsc{OpenAI Baselines} some PPO hyper-parameters that will not be study of our fine tuning tests: the SGA learning rate $\rho=3\times10^{-4}$, the lambda GAE parameter $\lambda=0.95$ and the  $L^\pi$ clip threshold\footnote{We refer to \textcite{ppo} for a more accurate description of $\epsilon$ and to \textcite{gae} for $\lambda$.} $\epsilon=0.2$. In every training phase, PPO builds two FFNNs with same architecture and activation functions to evaluate respectively the actions and the value function at each episode step $k$. The update of their hidden weights $\theta$ is performed after collecting a mini-batch of 2048 episodes.

The two FFNNs take in input the environment state \eqref{eq:state}. It is well known in literature that ANNs give better performances in the training phase if the input data are well normalized \citep{inputnormalization,normalizationinput}. Thus we manipulate the input state as follows:
\begin{equation}
	s_{T_k} = [T_k,X_{T_k}, I_{T_k}/I_0] \quad \forall T_k\in\mathcal{T},
\end{equation}
with 
	\begin{equation}
