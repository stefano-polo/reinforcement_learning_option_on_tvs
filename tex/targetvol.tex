...
In this work we study the constraint where the agent can invest in the risky market adopting long positions only:
\begin{equation}
	\alpha \cdot \mathds{1} = 1 \quad \alpha^i >0 \quad \forall i=1,\dots,n.
\end{equation} 
We study two different action parameterizations for this kind of constraint.
\begin{itemize}
	\item \textbf{$\bm{(n-1)}$-spherical angles}: we rewrite $\alpha^j = (x^i)^2$ and we use $(n-1)$-spherical coordinates with unitary radius. Thus the strategy is parameterized by a vector of angles $\phi = (\phi^1,\dots,\phi^{n-1})^T$ as follows
	\begin{equation}
		\begin{aligned}
			\alpha^1 &\coloneqq \cos^2 (\phi^1), \\
			\alpha^2 &\coloneqq \sin^2(\phi^1)\cos^2(\phi^2),\\
			&\vdots \\
			\alpha^{n-1} &\coloneqq \sin^2(\phi^1)\dots\sin^2(\phi^{n-2})\cos^2(\phi^{n-1}),\\
			\alpha^n &\coloneqq \sin^2(\phi^1)\dots\sin^2(\phi^{n-2})\sin^2(\phi^{n-1}).
		\end{aligned}
		\label{eq:euler_angles}\end{equation}
	where $\phi^i\in[0,\pi/2]$ to avoid oscillations in $\alpha$ that could slow down the training. Thus, with this parameterization, the ANN sample a vector of angles ($a = \phi$) and thus the action space is reduced by one dimension.
	...
	\begin{equation}
		\alpha_t = \frac{a_t}{a_t \cdot \mathds{1}}.
		\label{eq:normalized_action}\end{equation}
\end{itemize}



\section{Numerical Investigation}\label{sec:Numerical_results}
In this section As we have seen in 
