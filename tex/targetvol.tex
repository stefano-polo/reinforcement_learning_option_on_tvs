...
\begin{proposition}
When the underlying securities follow a Black and Scholes (BS) model with deterministic rates, the optimal strategy for a monotone payoff consists in minimizing the local drift function, independently of the current state $I_t$
\begin{equation}
...
\begin{equation}
	\alpha_{T_k} \cdot \mathds{1} = 1 \quad \alpha_{T_k}^i >0 \quad \forall i=1,\dots,n.
\end{equation} 
...
In this section we present the numerical results of our work. In particular we focus our analysis on an European option on the TVS with at-the-money strike considering two different models for the dynamics followed by the risky securities: Black and Scholes and local volatility. In \Cref{subsec:BS_model} we have proved that under the BS dynamics the solution of the control problem is given by \Cref{eq:BS_optimal_strategy}. We want to take advantage of this \textit{a priori} solution as a benchmark to gather evidence on the robustness of our RL approach and to check if our analytical result is correct. Moreover, the availability of this faster solution allows let us to identify the key parameters of the ANN, PPO and the action parameterization, so that to analyze how they impact on the final results and performances.  
Then we dedicate a full section to the LV case where the dynamical control problem must be solved by the RL agent.

In our work we keep fixed to the default values of \textsc{OpenAI Baselines} some PPO hyper-parameters that will not be study of our fine tuning tests: the SGA learning rate $\rho=3\times10^{-4}$, the lambda GAE parameter $\lambda=0.95$ and the  $L^\pi$ clip threshold\footnote{We refer to \textcite{ppo} for a more accurate description of $\epsilon$ and to \textcite{gae} for $\lambda$.} $\epsilon=0.2$. In every training phase, PPO builds two FFNNs with same architecture and activation functions to evaluate respectively the actions and the value function at each episode step $k$. The library initialize the hidden weights $\theta$ using an orthogonal initialization with $\sqrt{2}$ scaling factor and  update them after collecting a mini-batch of 2048 episodes. The network biases are set to zero.
...
where $F(t,T)$ is the forward curve vector of the risky assets from $t$ to $T$. With the normalization procedure \eqref{eq:input_normalization}  allows to remap $S_{T_K}$ in a standard Gaussian variable for the BS model, while in a proxy of it in the case of a LV dynamics. We adopt this normalization without loss of generality as the variables involved are observables (forward curve and at-the-money volatilities), as they are values that are always accessible by the fund manager in the market. 
...
We use the BS environment as toy model to understand which parameters of the RL algorithm play key roles in the training and testing phase. We also study which action parameteriziation is most effective in the case of a long only allocation strategy $\alpha$. We consider a target volatility call option with the following contract details between bank and fund manager
\begin{equation}\nonumber
...
--dire perchè black e del fine tuning dei parametri 
