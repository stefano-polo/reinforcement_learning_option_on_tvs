...
\begin{abstract}
We deal with \remove{the funding costs} \change{a stochastic optimal control problem} rising from hedging the risky securities underlying a target volatility strategy (TVS), a portfolio \remove{of risky assets and a risk-free one dynamically rebalanced in order to keep the} \change{whose asset-allocation is adjusted to maintain the} realized volatility of the portfolio on a certain level. \change{We consider the point of view of a derivative writer selling an option contract as protection to a portfolio manager on the capital invested in a TVS.} The uncertainty in the TVS risky portfolio composition along with the difference in hedging costs \change{of its} \remove{for each} component requires to \remove{solve a control problem to evaluate the option} \change{adjust the protection} price\remove{s} \change{to include these costs in the worst-case scenario for the seller}. We derive an analytical solution of the problem in the Black and Scholes (BS) scenario. Then we use Reinforcement Learning (RL) techniques to determine the fund composition leading to the \remove{most conservative price} \change{optimal policy} under the local volatility (LV) model, for which an {\it a priori} solution is not available. We show how the performances of the RL agents are compatible with those obtained by applying path-wise the BS analytical strategy to the TVS dynamics, which therefore appears competitive also in the LV scenario.
...
In the recent literature\change{,} \remove{TVSs} \change{TVS-based portfolios} are tested to investigate their performances in term of realized returns, see for instance \cite{Hocquard2013} and \cite{Perchet2016}, and the soundness of the volatility targeting algorithm, as described in \cite{Kim2018}\change{, where the authors propose the use of artificial neural networks for volatility forecasting volatility to enhance the performance of an asset allocation strategy}. 

\remove{Moreover, in the present day, in the derivative market new structured financial products that see TVSs as underlying are becoming widely used by practitioners.} \change{The success of TVS-based portfolios among investors has led to the emergence of new structured products with such portfolios as underlyings}. Those derivative instruments are known as target volatility options (TVOs) \change{and have become increasingly important in the derivative market.} \change{For these reasons TVOs}  have gained great interest in the derivative pricing literature \change{and research}, see in particular \cite{Albeverio2019}, \cite{DiGraziano2012}, \cite{DiPersio2019}, and \cite{Grasselli2016}. 

This paper \change{aims to enrich the pricing framework of the TVOs by studying for the first time,} \remove{is the first attempt} to the best of our knowledge, \remove{to study} the funding costs coming from hedging the risky assets underlying the target volatility strategy. In particular, we consider the point of view of \remove{a bank} \change{an option writer (for example a bank)} selling a call option to a portfolio manager as protection on the capital invested in a TVS. \change{During her activity the} \remove{The} portfolio manager has the freedom of changing the relative weights of the risky assets during the life of the TVS \change{depending on how the market evolves. This mechanism is no regulated by the contract, as the manager has decision making power on which asset invest}. Since \change{the fund manager's strategy cannot be known {\it a priori} and since} the risky assets have different hedging costs, the \remove{bank} \change{option writer} shall adjust the price of the protection to include \remove{them} \change{these costs} in the worst-case scenario, i.e. the most expensive strategy in term of the financing costs. Hence, the pricing problem becomes a continuous dynamical control problem over the risky portfolio composition\change{: determine the manager's optimal strategy that leads to the maximum protection price}. 

In our work we provide to the reader a formal description of the stochastic control problem \change{and we show that it can be solved only numerically considering a general state-dependent dynamics for the risky assets.} \change{However, we prove that} \remove{and most importantly we derive} an analytical solution to the problem \change{exists,} assuming that the risky asset dynamics underlying the TVS  follow a Black and Scholes (BS) model \cite{BlackScholes1973} and that the derivative contract is a European-style option. \change{As our first contribution, we derive the BS closed solution into two different ways: applying the Gy\"ongy Lemma and the other through the Hamilton-Jacobi-Bellman equation.} 
...
\end{equation}
\end{proposition}
The absence of stochastic elements in Equation~\eqref{eq:BS_optimal_strategy} makes the optimal strategy known \textit{a priori} with no \change{numerical} simulation needed \change{to solve the control problem}; in fact \remove{one can solve the optimization problem once for all $\forall t \in \mathcal{T}$ just looking at the market data $\mu(t)$ and $\nu(t)$ for the securities} \change{, once the risky-assets market data $\mu(t)$ and $\nu(t)$, it is sufficient to apply Equation~\eqref{eq:BS_optimal_strategy} $\forall t \in \mathcal{T}$  to retrieve the deterministic optimal strategy}. Once $\alpha^*$ is known, then one can price the payoff by the following BS formula
\begin{equation}
...
In our work we adopt two learning strategies to compare their performances in terms of overall reward: \remove{an \textit{ad hoc}} \change{a plain} direct policy \change{search} algorithm and the state of the art proximal policy optimization (PPO) developed in \cite{Schulman2017} and \cite{Schulman2016}. 

The first method is \change{well-known in RL literature \cite{Sutton2018}: it is based on the direct update of the agent policy via a gradient-based optimization of the value function.}\remove{a specific algorithm developed by us to fit the problem aim to find the optimal option price; this technique can take place as a direct policy optimization method in the wide taxonomy of Reinforcement Learning algorithms}. We will go into details in Section~\ref{sec:direct_policy}.
...
\subsection{Direct \change{P}olicy \change{Search} \change{A}pproach}\label{sec:direct_policy}
...
\end{equation}  
where the expectation $\hat{\mathbb{E}}[\dots]$ indicates the empirical average on the batch. Then the parameters are updated by plugging the policy into a stochastic gradient ascent algorithm \change{with the aim of maximizing Equation~\eqref{eq:direct_policy_loss}}. \remove{The choice of Equation~\eqref{eq:direct_policy_loss} is justified by the fact that, when the algorithm will find the optimal set $\theta^*$, then $L(\theta^*)$ will be a good proxy of the optimal option price.} 
