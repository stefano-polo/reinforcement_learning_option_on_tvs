	...
	Kuttelgasse 7, 8001 Z\"urich, Switzerland\\
	\email{Nastasi@swissquant.com} \and Financial Engineering,
	X-Numeris S.r.l.,\\ Via Turro, 7 , 20127 Milan, Italy\\
	\email{Sp.Stefano.Polo@gmail.com}}
	...
	\noindent {\bf Keywords.} Stochastic control problem, reinforcement learning, target volatility, option pricing, hedging costs.
	...
	In this algorithm, we parameterize the agent policy with a feed forward neural network (FFNN), such that $\theta_j$ coincides with the hidden weights, $s_k$ with the input neurons, and $a_k$ with the output \change{ones}. In this way, we are dealing with a deterministic policy, \change{that maps each state directly to a single action without any additional randomness or probability distribution;} \remove{where its} \change{the map} functional form is given by the neural network. The parameters update is performed as follows: the agent collects a finite batch of experiences interacting with the environment in a set of episodes $\tau$, then the loss-\change{value} function is evaluated as
	\begin{equation}
