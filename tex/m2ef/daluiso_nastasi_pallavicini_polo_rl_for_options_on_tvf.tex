...
\title{Reinforcement \change{L}earning for \change{O}ptions\\on \change{T}arget \change{V}olatility \change{F}unds\thanks{Supported by ISCRA from Cineca, \email{iscra@cineca.it}.}}
...
In Figure~\ref{fig:strategy_price_comparison} we provide a comparison among the option price obtained with the optimal strategy (BS$^*$) of Equation~\eqref{eq:BS_optimal_strategy} and those with \change{the below} \remove{other} intuitive strategies (S$_A$, S$_B$, S$_C$). Here we consider the case of an at-the-money call option with spot $I_0 = 1\, \text{EUR}$, maturity $T=5 \, \text{yr}$, target volatility $\bar{\sigma}=5\%$, and a nonnegative constraint on $\alpha$. The intuitive strategies are
\begin{itemize}
...
\end{equation}
if the payoff is non-decreasing in $I$ by homogeneity of the SDE we get that $V$ is non-decreasing\change{.}\remove{; t}\change{T}hus the solution is given by
\begin{equation}
